...
fn log_level_filtering() {
  let output = run_target("emit_log_levels", &[("RUST_LOG", "info")]);
  ...
  assert!(!output.contains(LOG_DEBUG), "debug should be filtered out");
...
fn trace_level_filtering() {
  ...
  let output = run_target("emit_trace_levels", &[("RUST_LOG", "debug")]);
  assert!(output.contains(TRACE_ERROR), "missing error");
  assert!(output.contains(TRACE_WARN), "missing warn");
  assert!(output.contains(TRACE_INFO_LEVELS), "missing info");
  assert!(output.contains(TRACE_DEBUG), "missing debug");
}
